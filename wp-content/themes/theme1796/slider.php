	...
	jQuery(window).load(function() {
		jQuery(function(){
			jQuery('.camera_wrap').camera({
				...
				height				: '45.26%',	//here you can type pixels (for instance '300px'), a percentage (relative to the width of the slideshow, for instance '50%') or 'auto'
				...
				navigation			: true<?php //echo of_get_option('sl_dir_nav'); ?>,	//true or false, to display or not the navigation buttons
				navigationHover		: true<?php //echo of_get_option('sl_dir_nav_hide'); ?>,	//if true the navigation button (prev, next and play/stop buttons) will be visible on hover state only, if false they will be visible always 
				pagination			: <?php echo of_get_option('sl_control_nav'); ?>,
				playPause			: true<?php //echo of_get_option('sl_play_pause_button'); ?>,	//true or false, to display or not the play/pause buttons
				pieDiameter			: 38,
		...
		<?php echo "data-src='";
					...
					<div class="camera_caption <?php echo $banner_animation;?>" data-easing="easeOutBack">
                        <div class="caption_link">
                            <span style="float:left; width: 80%"><?php echo stripslashes(htmlspecialchars_decode($caption[0])); ?></span><span style="float:left; margin-left: 20px;"><a href="<?php echo $url[0]?>"><img src="<?php bloginfo('stylesheet_directory'); ?>/images/see-more.png" border="0" /></a></span>
                        </div>
					</div>
					<?php } ?>
		<?php }else{ ?>
