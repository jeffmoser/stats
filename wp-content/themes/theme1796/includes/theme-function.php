...
// JM - edited to prevent ending in the middle of a word, added elipse at the end.
function my_string_limit_char($excerpt, $substr=0)
{
	...
	if ($substr>0) {
        ...
        $string = substr($string, 0, strrpos(substr($string, 0, $substr), ' '));
	}
	return $string.'... ';
		}
...
function theme1796_random($length){
...
add_action('manage_slider_posts_custom_column' , 'custom_slider_columns', 10, 2);
add_filter('manage_edit-slider_columns', 'my_slider_columns');
//Add columns for slider posts
...
function custom_slider_columns( $column, $post_id ) {
	switch ( $column ) {
	case 'slider_categories':
		$terms = get_the_term_list( $post_id , 'slider_category' , '' , ',' , '' );
		if ( is_string( $terms ) ) {
			echo $terms;
		} else {
			echo 'Uncategorized';
...
function custom_portfolio_columns( $column, $post_id ) {
	switch ( $column ) {
	case 'portfolio_tags':
		$terms = get_the_term_list( $post_id , 'portfolio_tags' , '' , ',' , '' );
		if ( is_string( $terms ) ) {
			echo $terms;
		} else {
			echo 'No Tags';
	...
	<li <?php comment_class(); ?> id="li-comment-<?php comment_ID() ?>">
		<div id="comment-<?php comment_ID(); ?>" class="comment-body">
			<div class="comment-author vcard">
				<?php echo get_avatar( $comment->comment_author_email, 84 ); ?>
			</div>
			...
			<div class="wrapper">
				<?php printf(__('<span class="author-comment">%1$s</span>' ), get_comment_author_link()) ?>
				<div class="reply">
