		...
		foreach($latest as $post) {
				...
				if ($thumb == 'true') {
						if ( has_post_thumbnail($post->ID) ){
							...
							if($type == 'services'){
								$output .= '<h4>';
									$output .= get_post_meta($post->ID, 'subtitle', array( "class" => "custom_field" ));
								$output .= '</h4>';
							}
							if($meta == 'true'){
...
function shortcode_audio($atts, $content = null) {
    ...
    if($mp3 !== false){
        ...
        _e("The URL you entered is not a .mp3 file.", "theme1796");
...
function shortcode_video($atts, $content = null) {
    ...
    if($vimeo !== false){
        ...
        _e("You entered a video URL that isn't compatible with this shortcode.", "theme1796");
